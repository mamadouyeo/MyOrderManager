...
// Route pour ajouter une imprimante avec une image
const AddImprimante = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { marque, purchasePrice, typeImprimante, color } = req.body;

    // Validation des champs requis
    if (!marque || !purchasePrice || !typeImprimante || !color) {
    ...
    const savedImprimante = await newImprimante.save();
